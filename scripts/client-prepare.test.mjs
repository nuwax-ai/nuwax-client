...
import { prepare, resourceSpecs, inputDigest, validatePinnedFrontend, packageReady, sourcePayloadReady } from './client/prepare.mjs';
...
function fixture(t, { platform = 'darwin', arch = 'arm64' } = {}) {
  ...
  const tools = {
    ...
    npmRun(dir, script) {
      ...
      if (script === 'build' && dir.includes(path.join('.cache', 'client-toolchain', 'sources'))) {
        write(path.join(dir, 'dist/index.js'));
        if (path.basename(dir).startsWith('nuwax-file-server-')) write(path.join(dir, 'dist/server.js'));
      }
...
test('preparation orders agent-kit before workspace and reuses unchanged inputs', async (t) => {
  ...
  assert.ok(f.calls.some((call) => call[0] === 'pnpm' && call[1] === f.kit && call.includes('--no-frozen-lockfile')));
  assert.ok(f.calls.some((call) => call[0] === 'run' && call[1] === 'git' && call[2] === 'clone' && call.includes('--depth')));
  f.calls.length = 0;
  await prepare(f.root, options);
...
test('agent-kit uses frozen install when its own lock exists', async (t) => {
  ...
  assert.equal(install.includes('--no-frozen-lockfile'), false);
});

test('agent-kit dependency changes refresh ignored lock before inherited frozen postinstall', async (t) => {
  const f = fixture(t);
  const lockFile = path.join(f.kit, 'pnpm-lock.yaml');
  const packageFile = path.join(f.kit, 'package.json');
  const pnpmRun = f.tools.pnpmRun;
  const expected = () => JSON.stringify(core.readJson(packageFile).dependencies ?? {});
  f.tools.pnpmRun = (dir, args) => {
    if (dir === f.kit && args[0] === 'install') {
      assert.ok(args.includes('--no-frozen-lockfile'));
      write(lockFile, expected());
    }
    if (dir === f.p.base && args[0] === 'install') assert.equal(fs.readFileSync(lockFile, 'utf8'), expected(), 'inherited CI=true install sees matching lock');
    return pnpmRun(dir, args);
  };
  const options = { tools: f.tools, platform: f.platform, arch: f.arch };
  await prepare(f.root, options);
  write(packageFile, { packageManager: 'pnpm@9.15.5', dependencies: { example: '2.0.0' } });
  await prepare(f.root, options);
  assert.equal(fs.readFileSync(lockFile, 'utf8'), '{"example":"2.0.0"}');
  f.calls.length = 0;
  await prepare(f.root, options);
  assert.equal(f.calls.filter((call) => ['pnpm', 'npm'].includes(call[0])).length, 0);
});

test('file-server CLI payload supports stale main metadata and requires real server entry', (t) => {
  const f = fixture(t);
  const directory = path.join(f.root, 'actual-file-server');
  write(path.join(directory, 'package.json'), { name: 'nuwax-file-server', main: 'index.js', bin: { 'nuwax-file-server': './dist/cli.js' } });
  write(path.join(directory, 'dist/cli.js'), 'CLI entry');
  write(path.join(directory, 'dist/server.js'), 'server entry');
  assert.equal(packageReady(directory), true);
  assert.equal(sourcePayloadReady('nuwax-file-server', directory), true);
  fs.rmSync(path.join(directory, 'dist/server.js'));
  assert.equal(sourcePayloadReady('nuwax-file-server', directory), false);
});
