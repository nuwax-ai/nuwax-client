...
### 十.5 cua-* 外部落盘内聚治理（2026-09-19，补丁 3，随 v1.0.18）

用户定调：驱动自有落盘一律内聚 Nuwax 体系（~/.nuwax/computer-use），机器不出现 cua-* 外部目录。上游四处外部写（mac-mini 实测两组）——pid 文件（~/Library/Caches/cua-driver/）、~/.cua-driver/ 遥测·安装标记、libei restore token（~/.config/cua-driver/）、history（App Support/<namespace>/computer-history）、浏览器 profile（App Support/CuaDriver/BrowserProfiles 等）——经**补丁 3 统一 env `CUA_DRIVER_DATA_HOME`** 收编：`user_home_subdirectory()` 返回绝对路径使全部 `home.join()` 调用点按 Path 语义替换前缀（一处改动收编 telemetry/version_check/release_channel/skills/doctor），pid/token/history/browser-profile 各自接入同开关（telemetry 既有 CUA_DRIVER_TELEMETRY_HOME 与 browser 既有 CUA_DRIVER_BROWSER_PROFILE_ROOT 保持次级优先）；未设 env 上游行为不变。壳侧 launchDaemon 三平台注入（mac 走 `open --env`，win/linux spawn env），目标目录与 nuwax.db 同源（APP_DATA_DIR_NAME 派生）。**容器实证**：serve 带 env 后 pid 落 `$CUA_DRIVER_DATA_HOME/cua-daemon.pid`、外部 ~/.cache/cua-driver 零生成、握手 60 工具不变。旧残留不清理（~/Library/Caches/cua-driver 与官方 CuaDriver 共用 namespace 勿删；~/.cua-driver 旧标记无害）。补丁文件升级为 docs/computer-use-poc/0001-0003-cua-nuwax-helper-bundle.patch（cua 分支尖端 50b4881）。
